...
### Requirement: Submit resolves reviewed rows and sends their invite
The system SHALL, for each reviewed row an Admin submits, resolve it into a real player account and send that player their first login-link invite in the same action, and SHALL report each row's outcome independently so one row's failure does not block the others.

#### Scenario: Submitting a fully reviewed row succeeds
- **WHEN** an Admin submits a row with a confirmed name, email, and phone
...
- **WHEN** an Admin submits multiple reviewed rows and one row's resolution fails
