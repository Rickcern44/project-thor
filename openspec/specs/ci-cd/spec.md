...
Automated CI gating for pull requests and a GitOps deployment path from merged code to the homelab Kubernetes cluster: independent, per-service versioned Docker image builds published to `ghcr.io` on merge to `main`, and per-service Kustomize manifests synced automatically by ArgoCD.
